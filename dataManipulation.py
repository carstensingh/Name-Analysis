...
def topXinY(filename, n, year):  #returns top x names in year in given file
  ...
  with open(filename, 'r', encoding='utf-8', errors='ignore') as File: 
    ...
    if (int(count) < int(n)):
      ...
      print("Most popular names in " + str(year))
      print('\033[95;1m'+ f'{"Rank"} {"  "} {"Years "} {" Names":<{maxlength}} {"Frequency":<17}'+'\033[;0m')
      for i in range(0, int(n)):
        print(f'{i+1:<7} {years[i]:<7} {names[i]:<{maxlength}}{"   "}{freq[i]}')
        topXfreq.insert(0, freq[i])
        topXname.insert(0, names[i])
...
def observeName(filename):
  ...
  with open(filename, encoding="ISO-8859-1") as csvDataFile:  # open csv file
    ...
    for row in csvReader:  # iterate over rows
      ...
      if row[1].title() == personName.title():  # if the name matches the name in the command line arguement
        nameFound = 1
    ...
    if (nameFound == False):  # check if the name was found in the csv file
      ...
      print('\033[95;1m'+ f'{"Years   "} {" Names":<{10}} {"Frequency":<17}'+'\033[;0m')
      for i in range(len(peopleList)):  # print out the year and frequency for the given name
        print((f"{peopleList[i][0]:<7} {peopleList[i][1]:<15} {peopleList[i][3]:<5}"))
  print("\n")
  xlabel = 'Years'
  ylabel = 'Frequency'
...
def observeLetter(filename):
  ...
  if (letterFound == 0):  # if letter was not found then let the user know
    ...
    try:
      ...
      graphMenu(name[-20:], counts[-20:], xlabel, ylabel, title)
    except:
      peopleList = peopleList[-len(peopleList):]
      for l in range(len(peopleList)-1,0,-1):
...
def compareTopTen(inputFileName, inputFileNameTwo, year, loc1, loc2):
  ...
  if (len(names) > 1):
    if(("newfoundland" not in inputFileName) and ("newfoundland" not in inputFileNameTwo)):
      for i in range(0, 11):
        if (i == 0):
          print("\n\n" + '\033[95;1m' + f'{"    "} {names[0]:<{maxlength1}}   {"  Frequency   "} {names2[0]:<{maxlength2}} {"     Frequency     "}' +'\033[;0m')
        else:
      ...
      for i in range(0, 11):
        if (i == 0):
          print("\n\n" + '\033[95;1m' +
                f'{"    "} {names[0]:<{maxlength1}}{names2[0]:<{maxlength2}}' +'\033[;0m')
        else:
          print(f'{i:<4} {names[i]:<{maxlength1}} {names2[i]:<{maxlength2}} ')
