  ...
  if (gender != 'Both'):
    ...
    with open(filename) as csvDataFile:
      ...
      for row in csvReader:
        if (row[1] == gender): #check the gender
          years.append(row[0])
          ranks.append(-1)
          names.append(row[3].title())
          count.append(int(row[4]))
          total = total + 1
    people = {'Year': years, 'Name': names, 'Frequency': count, 'Rank': ranks} #make a dataframe
    ...
    sorted_people['Rank'] = sorted_people.groupby('Year')['Frequency'].rank(ascending = False, method = 'dense').astype(int) #sort the rank
    #print(sorted_people)
    if gender == 'Female':
